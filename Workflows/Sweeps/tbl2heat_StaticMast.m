function tbl2heat_StaticMast(resultsTable,plotStyle,varargin)
% For use with Step Steer Tests
% Use with: StaticMastLoadSweep
...
%            'Title',['Lift = ' num2str(MastLiftPosList(mi)) ', Tilt = ' num2str(MastTiltAngleList(ti))],Colormap=[0 1 0;1 0 0]);
    ...
    if(strcmp(plotStyle,'surface'))
        ...
        title(['Static CG Test (Lift=' num2str(MastLiftPosList(mi)) ' mm): Min Wheel Normal Force'])
        if(~isempty(fileName))
...
if(strcmp(plotStyle,'heatmap'))
    title(t,'Static CG Test: Min Wheel Normal Force')
...
if(~isempty(fileName))
    writematrix(resHMs,[folderName filesep 'testSweep_' fileName(end-10:end) '.xlsx'],'Sheet','Static','Range','B2');
end
