function tbl2heat_constRad(resultsTable,testConds,plotStyle,varargin)
% For use with Constant Radius Tests
% Four combinations: ForwardVelocitySweep ReverseVelocitySweep
...
SteerAngleList    = unique(resultsTable.SteerAngle);
...
for si = 1:length(SteerAngleList)
    figure
    ...
    for mi = 1:length(MastLiftPosList)
        ...
        for ti = 1:length(MastTiltAngleList)
            rows = (resultsTable.MastLiftPos==MastLiftPosList(mi)) & (resultsTable.MastTiltAngle==MastTiltAngleList(ti)) ...
                & (resultsTable.SteerAngle==SteerAngleList(si));
        ...
        if(strcmp(plotStyle,'surface'))
            ...
            title(['Constant Radius (' num2str(testConds.vel) 'm/s, Lift=' num2str(MastLiftPosList(mi)) 'mm, Steer=' num2str(SteerAngleList(si)) 'deg): Min Wheel Fn'])
            if(~isempty(fileName))
                figFileName = [fileName '_Surf'];
            end
            if(~isempty(fileName))
                exportgraphics(gcf,[folderName filesep figFileName '_Lift' num2str(mi) '_qS' num2str(SteerAngleList(si)) '.png'],"Resolution",100)
            end
        end

    end
    if(strcmp(plotStyle,'heatmap'))
        title(t,['Constant Radius Test (' num2str(testConds.vel) 'm/s, ' num2str(testConds.steer(si)) ' deg): Min Wheel Normal Force']);
    ...
    ylabel(t,['Load: ' result])
        ...
        if(~isempty(fileName))
            %saveas(gcf,[folderName filesep fileName '_qS' num2str(SteerAngleList(si)) '.png'])
            exportgraphics(gcf,[folderName filesep fileName '_qS' num2str(SteerAngleList(si)) '.png'],"Resolution",100)
        end
    end
    if(~isempty(fileName))
        writematrix(resHMs,[folderName filesep 'testSweep_' fileName(end-10:end) '.xlsx'],'Sheet',['ConstRad_' FwdOrRev '_' num2str(SteerAngleList(si))] ,'Range','B2');
    end
end
